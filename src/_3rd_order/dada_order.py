...
class DaDaOrder(Base3rdOrder):
    ...
    def get_total_order_amount(self):
        total_order_amount = self.order_data[self.order_data['订单状态'] == '已完成']['运费账户消耗'].sum()
        return round(total_order_amount, 2)
    
    def get_total_penalty_amount(self):
        total_penalty_amount = self.order_data[self.order_data['订单状态'] == '已取消']['违约金'].sum()
        return round(total_penalty_amount, 2)

    def get_order_number_info(self, order_number):
        data = self.order_data[self.order_data['第三方订单ID'] == order_number]

        if not data.empty:
            order_id = data.iloc[0]['达达订单ID']
            orider_status = data.iloc[0]['订单状态']
            order_amount = float(str(data.iloc[0]['运费账户消耗']).replace(',', '').strip())
            order_penalty = float(str(data.iloc[0]['违约金']).replace(',', '').strip())
...
if __name__ == "__main__":
    file_path = os.path.join(os.path.dirname(__file__), '..\..', 'data', '达达.xlsx')

    order = DaDaOrder(file_path)

    order_number = '3901130812901951564_6gn45nx5lcj' #4039770077615541847_a3e0nmye2d5 hxbcblzorjfyzo23x_a0l84m0od9z
